...
// Copyright (c) 2016 The KMC developers
...
const string strMessageMagic = "KMC Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // KMC: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // KMC: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // KMC: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// KMC: find block wanted by given orphan block
...
// KMC: minter's coin stake is rewarded based on coin age spent (coin-days)
...
// KMC: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // KMC: target change every block
    // KMC: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // KMC: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // KMC: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // KMC: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // KMC: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // KMC: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // KMC: track money supply and mint amount info
    ...
    // KMC: fees are not collected by miners as in bitcoin
    // KMC: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// KMC: total coin age spent in transaction, in the unit of coin-days.
...
// KMC: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // KMC: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // KMC: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // KMC: record proof-of-stake hash value
...
bool CBlock::CheckBlock() const
{
    ...
    // KMC: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // KMC: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // KMC: check switch from proof of work to proof of stake
    ...
    // KMC: check that the block satisfies synchronized checkpoint
    ...
    // KMC: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // KMC: verify hash target and signature of coinstake tx
    ...
    // KMC: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // KMC: getblocks may not obtain the ancestor block rejected
    ...
    // KMC: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "KMC", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "KMC", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        printf("KMC Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();
     
        printf("End KMC Genesis Block\n");
        ...
        // KMC: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // KMC: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // KMC: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // KMC: should not enter safe mode for longer invalid chain
    // KMC: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 90) && !fTestNet)
    {
        nPriority = 100;
		strStatusBar = "";
        //strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/KMC/KMC/issues).";
    }

    // KMC: if detected invalid checkpoint enter safe mode
    if (Checkpoints::hashInvalidCheckpoint != 0)
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/KMC/KMC/issues).";
    }
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // KMC: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // KMC: record my external IP reported by peer
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // KMC: immediate retry during initial download
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // KMC: send latest proof-of-work block to allow the
                        // download node to accept as orphan (proof-of-stake 
                        // block might be rejected by stake connection check)
                        // KMC: send latest block
                        vector<CInv> vInv;
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // KMC: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // KMC: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // KMC: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("KMCMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KMCMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("KMCMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("KMCMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // KMC: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running KMCMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadKMCMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadKMCMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadKMCMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d KMCMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadBitcoinMiner, pwallet))
                printf("Error: CreateThread(ThreadKMCMiner) failed\n");
            Sleep(10);
        }
    }
}
