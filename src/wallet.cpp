...
// KMC: optional setting to unlock wallet for block minting only;
...
// KMC: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // KMC: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // KMC: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    //  post-backup change.
#ifdef WIN32
                    if (!GetBoolArg("-avatar", true)) // KMC: not avatar mode; kmc: avatar mode enabled by default to avoid change being sent to hidden address
#else
...
// KMC: create coin stake transaction
...
// KMC: check 'spent' consistency between wallet and txindex
// KMC: fix wallet spent state according to txindex
...
// KMC: disable transaction (only for coinstake)
