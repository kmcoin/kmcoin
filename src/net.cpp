...
{
    // Wait for IRC to get it first - disabled with kmc
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "kmc " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are kmc dns seeds.
static const char *strDNSSeed[][3] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real kmc offering, you will want to remove the "t" that
    // appears before the node name and IP
      {"139.196.9.24","139.196.10.49","139.196.24.155"},{"139.196.9.24","139.196.10.49","139.196.24.155"}
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // KMC enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  kmc is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    if(GetBoolArg("-genpos", false)){
        // KMC: mint proof-of-stake blocks in the background
