...
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "kmc";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-conf", "kmc.conf"));
...
void ReadPeercoinConfigFile(map<string, string>& mapSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No KMC config file is OK
...
boost::filesystem::path GetPidFile()
{
    ...
    fs::path pathPidFile(GetArg("-pid", "kmc.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong kmc will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("kmc"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "KMC:" << FormatVersion(PEERSHARES_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) / "kmc.lnk";
}
...
{
    return GetAutostartDir() / "kmc.desktop";
}
